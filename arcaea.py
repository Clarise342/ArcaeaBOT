...
ignorepacks = {"Ae":"Archive","Aa":"Arcaea","WE":"World Extend","BF":"Black Fate","AP":"Adverse Prelude","LS":"Luminous Sky","VL":"Vicious Labyrinth","EC":"Eternal Core","EP":"Ephemeral Page","SR":"Sunset Radiance","AR":"Absolute Reason","BE":"Binary Enfold","Am":"Ambivalent Vision","CS":"Crimson Solace","O.":"O.N.G.E.K.I.","CH":"CHUNITHM","GC":"Groove Coaster","TS":"Tone Sphere","La":"Lanota","Dx":"Dynamix"} # ignore_packs
sides = {"all":None,"light":"光","conflict":"対立"} # side
composers = data["composers"]
illustrators = data["illustrators"]
chart_creators = data["chartcreator"]
...
skills = {"-":"-","E":"Easy","H":"Hard","V":"Visual","M":"ミラー","O":"オーバーフロー","C":"チュウニズム","A":"Audio","O.":"O.N.G.E.K.I."} # skills
...
async def help(ctx):
  ...
  e.add_field(name="Other",value="BOTの導入は[こちら](https://discord.com/api/oauth2/authorize?client_id=702587324718120991&permissions=103935056&scope=bot)\n公式サーバーは[こちら](https://discord.gg/zgjxKHBteH)",inline=False)
...
async def latest(ctx):
  await ctx.message.edit(delete_after=2.0)
  e = dc.Embed(title="v1.2 | v3.4に対応しました",color=0x74596d)
  e.timestamp = dt.utcnow()
  e.add_field(name="◆ パックの追加",value="新パック「O.N.G.E.K.I.」を追加",inline=False)
  e.add_field(name="◆ 曲の追加",value="O.N.G.E.K.I.パックの楽曲を追加",inline=False)
  e.add_field(name="◆ パートナーの追加",value="パートナー「ルナ&美亜」を追加",inline=False)
  e.add_field(name="◆ コマンドの修正",value="・`setting`コマンド内の文章を修正\n・設定更新時にメッセージを表示するよう変更",inline=False)
  e.add_field(name="◆ その他",value="・BOT招待リンクを更新(権限を再設定)\n・BOT公式サーバーリンクを表示(help)",inline=False)
      ...
      if page == 0:
        ...
        if msg.content == "back": 
          ...
          if msg.content in sfirst:
            ...
            await msg.channel.send(embed=dc.Embed(title="楽曲選択時の条件設定を初期化しました"), delete_after=5.0)
            page = 0
          else:
            await msg.channel.send(embed=dc.Embed(title="該当する設定項目はありません"), delete_after=5.0)
          ...
          if msg.content in ignorepacks:
            if ignorepacks[msg.content] in sopt["ignorepacks"]:
              ...
              await msg.channel.send(embed=dc.Embed(title=f"{ignorepacks[msg.content]} パックが除外対象から外されました"), delete_after=5.0)
            else:
              sopt["ignorepacks"].append(ignorepacks[msg.content])
              await msg.channel.send(embed=dc.Embed(title=f"{ignorepacks[msg.content]} パックが除外対象に追加されました"), delete_after=5.0)
            page = 1
          else:
            await msg.channel.send(embed=dc.Embed(title="該当するパックは見つかりませんでした"), delete_after=5.0)
        elif page == 3: # song_ignore_songs
          l = [s.name for s in sdata if msg.content in s.name]
          if len(l) >= 1:
            if l[0] in sopt["ignoresongs"]:
              del sopt["ignoresongs"][sopt["ignoresongs"].index(l[0])]
              await msg.channel.send(embed=dc.Embed(title=f"{l[0]} が除外対象から外されました"), delete_after=5.0)
            else:
              sopt["ignoresongs"].append(l[0])
              await msg.channel.send(embed=dc.Embed(title=f"{l[0]} が除外対象に追加されました"), delete_after=5.0)
            page = 1
          else:
            await msg.channel.send(embed=dc.Embed(title="該当する楽曲が見つかりませんでした"), delete_after=5.0)
          ...
          if msg.content in {5:["1","2","3","4","5","6"],6:["3","4","5","6","7","8","9"],7:["7","8","9","9+","10","10+","11"],8:["8","9","9+","10","10+","11"]}[page]:
            if msg.content in sopt["levels"][{5:0,6:1,7:2,8:3}[page]]:
              ...
              await msg.channel.send(embed=dc.Embed(title=f"難易度 {['PAST','PRESENT','FUTURE','BEYOND'][page-5]} の選曲対象からレベル {msg.content} が外されました"), delete_after=5.0)
            else:
              sopt["levels"][{5:0,6:1,7:2,8:3}[page]].append(msg.content)
              await msg.channel.send(embed=dc.Embed(title=f"難易度 {['PAST','PRESENT','FUTURE','BEYOND'][page-5]} の選曲対象にレベル {msg.content} が追加されました"), delete_after=5.0)
            page = 1
          else:
            await msg.channel.send(embed=dc.Embed(title="このレベルに該当するレベルの楽曲はありません"), delete_after=5.0)
        elif 11 <= page <= 14:
          splits = msg.content.split(" ")
          try:
            nlvalues = [int(x) for x in splits]
          except Exception:
            await msg.channel.send(embed=dc.Embed(title="無効な値が入力されました、もう一度入力して下さい"), delete_after=5.0)
          else:
            if len(nlvalues) == 2:
              sopt["notes_limit"][{11:0,12:1,13:2,14:3}[page]][0] = nlvalues[0]
              sopt["notes_limit"][{11:0,12:1,13:2,14:3}[page]][1] = nlvalues[1]
              await msg.channel.send(embed=dc.Embed(title=f"難易度 {['PAST','PRESENT','FUTURE','BEYOND'][page-11]} の選曲対象がノーツ数 {splits[0]} 〜 {splits[1]} に設定されました"), delete_after=5.0)
              page = 1
            else:
              await msg.channel.send(embed=dc.Embed(title="値が1つだけ入力されました、もう一度入力して下さい"), delete_after=5.0)
        elif page == 9:
          if msg.content in sides:
            sopt["side"] = sides[msg.content]
            await msg.channel.send(embed=dc.Embed(title=f"対象サイドが {msg.content} に設定されました"), delete_after=5.0)
            page = 1
          else:
            await msg.channel.send(embed=dc.Embed(title="無効な値が入力されました、もう一度入力して下さい"), delete_after=5.0)
        elif 20 <= page <= 22:
          ...
          if len(values) != 0:
            if values[0] in sopt[{20:"composers",21:"illustrators",22:"chart_creators"}[page]]:
              ...
              await msg.channel.send(embed=dc.Embed(title=f"対象{['作曲者','イラストレーター','譜面製作者'][page-20]}から {values[0]} が外されました"), delete_after=5.0)
            else:
              sopt[{20:"composers",21:"illustrators",22:"chart_creators"}[page]].append(values[0])
              await msg.channel.send(embed=dc.Embed(title=f"対象{['作曲者','イラストレーター','譜面製作者'][page-20]}に {values[0]} が追加されました"), delete_after=5.0)
            page = 1
          else: 
            await msg.channel.send(embed=dc.Embed(title=f"該当する{['作曲者','イラストレーター','譜面製作者'][page-20]}は見つかりませんでした"), delete_after=5.0)
          ...
          if msg.content in pfirst:
            ...
            await msg.channel.send(embed=dc.Embed(title="パートナー条件設定を初期化しました"), delete_after=5.0)
            page = 0
          else:
            await msg.channel.send(embed=dc.Embed(title="該当する設定項目はありません"), delete_after=5.0)                
        elif page == 24: # resident
          if msg.content in ["all","constant","limited"]:
            popt["resident"] = msg.content
            await msg.channel.send(embed=dc.Embed(title=f"選択対象を {msg.content} にしました"), delete_after=5.0)
            page = 23
          else:
            await msg.channel.send(embed=dc.Embed(title="該当する内容はありません"), delete_after=5.0)
        elif page == 25: # type
          if msg.content in types:
            if types[msg.content] in popt["types"]:
              del popt["types"][popt["types"].index(types[msg.content])]
              await msg.channel.send(embed=dc.Embed(title=f"タイプ {types[msg.content]} を選択対象から外しました"), delete_after=5.0)
            else:
              popt["types"].append(types[msg.content])
              await msg.channel.send(embed=dc.Embed(title=f"タイプ {types[msg.content]} を選択対象に追加しました"), delete_after=5.0)
            page = 23
          else:
            await msg.channel.send(embed=dc.Embed(title="該当するタイプは見つかりませんでした"), delete_after=5.0)
        elif page == 26: # skill
          if msg.content in skills:
            if skills[msg.content] in popt["skills"]:
              del popt["skills"][popt["skills"].index(skills[msg.content])]
              await msg.channel.send(embed=dc.Embed(title=f"スキル {skills[msg.content]} を選択対象から外しました"), delete_after=5.0)
            else:
              popt["skills"].append(skills[msg.content])
              await msg.channel.send(embed=dc.Embed(title=f"スキル {skills[msg.content]} を選択対象に追加しました"), delete_after=5.0)
            page = 23
          else:
            await msg.channel.send(embed=dc.Embed(title="該当するスキルは見つかりませんでした"), delete_after=5.0)
        elif page in [27,30]:
          ...
          try:
            ...
            if len(clvalues) == 2:
              if 16 <= page <= 19:
                ...
                await msg.channel.send(embed=dc.Embed(title=f"難易度 {['PAST','PRESENT','FUTURE','BEYOND'][page-16]} の選曲対象が譜面定数 {clvalues[0]} 〜 {clvalues[1]} に設定されました"), delete_after=5.0)
              elif 28 <= page <= 32:
                ...
                await msg.channel.send(embed=dc.Embed(title=f"{['FRAG(Lv.1)','FRAG(Lv.20)','dummy','STEP(Lv.1)','STEP(Lv.20)'][page-28]}の対象範囲が {clvalues[0]} 〜 {clvalues[1]} に設定されました"), delete_after=5.0)
...
async def on_ready():
  ...
  ch = bot.get_channel(787595201207992330)
